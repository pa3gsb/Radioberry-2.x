...
void handlePacket(char* buffer){
	...
	if (isValidFrame(buffer)) {
		...
		if ((buffer[11] & 0xFE)  == 0x14) {
			...
			att11 = att;
			pureSignal = ((buffer[11+2] & 0x40) == 0x40)? 0x04 : 0x00;
		}
		
		if ((buffer[523] & 0xFE)  == 0x14) {
			att = (buffer[523 + 4] & 0x1F);
			att523 = att;
			//pureSignal = ((buffer[523+2] & 0x40)==0x40)? 1 : 0;
		}
		...
		for (frame; frame < 2; frame++) {
			...
			for (i=0; i< (504 / (8 + factor)); i++) {
				...
				for (j; j< 6; j++){
					...
					if (aantal < 1000) fprintf(stderr, "%X", iqdata[j]);
				}
				if (aantal < 1000)  fprintf(stderr, "\nrx2: ");
				if (nrx > 1) {
					rx2_reader(iqdata);
					int j =0;
					for (j; j< 6; j++){
...
void spi_rx_writer() {
	...
	iqdata[0] = (0x40 | (pureSignal & 0x04)| (sampleSpeed & 0x03));
...
void spi_rx2_writer() {
	...
	iqdata[0] = (0x80 | (sampleSpeed & 0x03));
	iqdata[1] = (((rando << 6) & 0x40) | ((dither <<5) & 0x20) |  (att & 0x1F));
...
void *spiWriter(void *arg) {
	...
	while(1) {
		...
		//fprintf(stderr, "%X voor 0 \n" , tx_iqdata[0]);
		//fprintf(stderr, "%X voor 5 \n" , tx_iqdata[5]);
		 int txbufferc = ((tx_iqdata[4] & 0x07) << 8) + (tx_iqdata[5] & 0xFF);
