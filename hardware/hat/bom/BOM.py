...
class BOM:
    ...
    def LaTeXAssemblyPrint(self,pre="",prefer=None):
        ...
        for k in keys:
            ...
            c7 = p.Quantities(self.optionset)[0]
            if c7 == 0: continue
			
            c1 = LaTeXEscape(p.spec)
            c2 = p.LaTeXAssemblyLinks()
            c3 = p.sub
            c4 = p.LaTeXRefs(self.optionset)
            c5 = p.package
            c6 = p.pins

            items,parts,pins = ipp[p.assembly]
            ipp[p.assembly] = items+1,parts+c7,pins+(p.pins*c7)

            s = '{0} & {1} & {2} & {3} & {4} & {5} & {6} \\\\ \hline'.format(c1,c2,c3,c4,c5,c6,c7)
